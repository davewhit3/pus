   ...
   connect(sockfd, (struct sockaddr*) &remote_addr, addr_len);
    while(1){
     ...
     fgets (buff2, 100, stdin);
        if(buff2[0] == '\0' || buff2[0] == '\n'){
            fprintf(stdout,"Pusta wiadomosc! Wysylamy pusta do servera i konczymy dzialanie!: %s", buff2);
        }
        /* sendto() wysyla dane na adres okreslony przez strukture 'remote_addr': */
    retval = send(sockfd, buff2, sizeof(buff2), 0);
    if (retval == -1) {
        perror("sendto()");
        exit(EXIT_FAILURE);
    }
    ...
    retval = recv(sockfd, buff2, sizeof(buff2), 0);
    if (retval == -1) {
        perror("recvfrom()");
        exit(EXIT_FAILURE);
    }
